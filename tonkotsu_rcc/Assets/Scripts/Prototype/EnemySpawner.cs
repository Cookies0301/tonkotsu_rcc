...
public class EnemySpawner : MonoBehaviour
{
    ...
    private IEnumerator SpawnRoutine()
    {
        while (true)
        {
            ...
            if(Vector3.Distance(PlayerHandler.Player.position, transform.position) > range || EnemyHandler.Instance.CurrentEnemyCount > GameSettings.Instance.SavedData.maxEnemyCount)
            {
                continue;
            }
